...
int scale_process(void* inBuffer, int inWidth, int inHeight, void* outBuffer, int outWidth, int outHeight, int rotation, int fmt, float zoom)
{
    ...
    OMX_U32 w,h,zfactor;
    double aspect_ratio;
    int crop_width, crop_height, crop_xstart, crop_ystart;
    ...
    printf("crop_xstart = %d, crop_ystart = %d, crop_xsize = %d, crop_ysize = %d \n", crop_xstart, crop_ystart, crop_width, crop_height);
